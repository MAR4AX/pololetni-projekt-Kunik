...
Library library = new Library("Smíchov", colBooks, colExemplars, colUsers, colReaders, colEmployees, colVypujcky, colRezervace);
...
while (true)
{
    ...
    User loggedUser = colUsers
    .Find(u => u.contact == LogInContact && u.password == LogInPassword)
    .FirstOrDefault();
        ...
        switch (key.Key)
        {
            case ConsoleKey.M:
                ...
                else
                {
                    ...
                    Console.WriteLine($"pocet aktivnich vypujcek: {colVypujcky.CountDocuments(v => v.ctenar.Id == loggedUser.Id && !v.vraceno)}");
                    Console.WriteLine($"pocet aktivnich rezervaci: {colRezervace.CountDocuments(r => r.ctenar.Id == loggedUser.Id && !r.splnena)}");
                }
                Console.WriteLine();
            ...
            case ConsoleKey.V:
                ...
                var nalezeneKnihy = colBooks
                    .Find(b => b.name == search || b.author == search || b.ISBN == search || b.genre == search)
                    .ToList();

                foreach (var book in nalezeneKnihy)
                {
                ...
                if (!loggedUser.isEmployee)
                {
                    ...
                    if (Console.ReadKey().Key == ConsoleKey.Y)
                    {
                        ...
                        Book vybrana = nalezeneKnihy.FirstOrDefault(b => b.ISBN == zvoleneISBN);
                        if (vybrana == null)
                        {
                        ...
                        {
                            int aktualniPoradi = (int)colRezervace.CountDocuments(r => r.ISBN == zvoleneISBN && !r.splnena) + 1;
            ...
            case ConsoleKey.C:
                ...
                switch (klavesa.Key)
                {
                    case ConsoleKey.V:
                        ...
                        var readers = colReaders.Find(r => !r.isEmployee).ToList();
                    ...
                    case ConsoleKey.Z:
                        ...
                        User toDelete = colUsers.Find(r => r.readerNumber == delNum).FirstOrDefault();
                        if (toDelete != null)
                        {
            ...
            case ConsoleKey.K:
                ...
                switch (bookChoice.Key)
                {
                    ...
                    case ConsoleKey.O:
                        Console.Write("ISBN knihy k odebrání: "); 
                        string remISBN = Console.ReadLine();
                        Book toRemove = colBooks.Find(b => b.ISBN == remISBN).FirstOrDefault();
                        if (toRemove != null)
                        {
            ...
            case ConsoleKey.S:
                if (!loggedUser.isEmployee)
                    ...
                    Console.WriteLine($"pocet aktivních vypujcek: {colVypujcky.CountDocuments(v => !v.vraceno)}");
                Console.WriteLine($"pocet vypujcek po terminu: {colVypujcky.CountDocuments(v => !v.vraceno && DateTime.Today > v.datumVraceni)}");
                break;
    ...
    IMongoCollection<Rezervace> colRezervace)
{
    ...
    switch (volba.Key)
    {
        case ConsoleKey.N:
            ...
            User ctenar = colReaders.Find(r => r.readerNumber == karticky).FirstOrDefault();
        ...
        case ConsoleKey.R:
            ...
            Vypujcka vracena = colVypujcky.Find(v => v.exemplar.serial_number == serial && !v.vraceno).FirstOrDefault();
        ...
        case ConsoleKey.E:
            ...
            Vypujcka kProdlouzeni = colVypujcky.Find(v => v.exemplar.serial_number == serialProd && !v.vraceno).FirstOrDefault();
            ...
            bool maRezervaci = colRezervace.Find(r => r.ISBN == kProdlouzeni.exemplar.ISBN && !r.splnena).Any();
            if (maRezervaci)
            {
        ...
        case ConsoleKey.Z:
            ...
            var rezervace = colRezervace.Find(r => !r.splnena).ToList();
        ...
        case ConsoleKey.S:
            ...
            User sankCtenar = colReaders.Find(r => r.readerNumber == sankceKarticky).FirstOrDefault();
            if (sankCtenar == null)
            {
            ...
            var dluhy = colVypujcky.Find(v => v.ctenar.Id == sankCtenar.Id && v.pokuta > 0 && !v.pokutaZaplacena).ToList();
            if (dluhy.Count == 0)
            {
    ...
    IMongoCollection<Rezervace> colRezervace)
{
    ...
    switch (volba.Key)
    {
        case ConsoleKey.M:
            ...
            var mojVyp = colVypujcky.Find(v => v.ctenar.Id == ctenar.Id && !v.vraceno).ToList();
            ...
            {
                foreach (var vypujcka in colVypujcky.Find(_ => true).ToList())
                {
        ...
        case ConsoleKey.P:
            ...
            Vypujcka vyp = colVypujcky.Find(vyp => vyp.exemplar.serial_number == s && vyp.ctenar.Id == ctenar.Id && !vyp.vraceno).FirstOrDefault();
            if (vyp == null)
            {
            ...
            if (colRezervace.Find(r => r.ISBN == vyp.exemplar.ISBN && !r.splnena).Any())
            {
        ...
        case ConsoleKey.R:
            ...
            var mojRez = colRezervace.Find(r => r.ctenar.Id == ctenar.Id && !r.splnena).ToList();
            if (mojRez.Count == 0)
...
{
    public string name;
    private IMongoCollection<Book> colBooks;
    private IMongoCollection<Book> colExemplars;
    private IMongoCollection<User> colUsers;
    private IMongoCollection<User> colReaders;
    private IMongoCollection<User> colEmployees;
    private IMongoCollection<Vypujcka> colVypujcky;
    private IMongoCollection<Rezervace> colRezervace;

    public Library(string name,
    IMongoCollection<Book> colBooks,
    IMongoCollection<Book> colExemplars,
    IMongoCollection<User> colUsers,
    IMongoCollection<User> colReaders,
    IMongoCollection<User> colEmployees,
    IMongoCollection<Vypujcka> colVypujcky,
    IMongoCollection<Rezervace> colRezervace)
    {
        this.name = name;
        this.colBooks = colBooks;
        this.colExemplars = colExemplars;
        this.colUsers = colUsers;
        this.colReaders = colReaders;
        this.colEmployees = colEmployees;
        this.colVypujcky = colVypujcky;
        this.colRezervace = colRezervace;
    }
    ...
    public (Book, List<Book>) NewBook(string name, string author, string ISBN, int year, string genre, int exemplars, string serial_number)
    {
        ...
        for (int i = 0; i < book.exemplars; i++)
        {
            ...
            colExemplars.InsertOne(exemplar);
    ...
    {
        User user = new User(name, surname, false, (int)colReaders.CountDocuments(r => !r.isEmployee) + 1, contact, password);
        colReaders.InsertOne(user);
    ...
    public void NewEmployee(string name, string surname, bool isEmployee, int readerNumber, string contact, string password)
    {
        ...
        colEmployees.InsertOne(user);
        colUsers.InsertOne(user);
    }

    public Book NajdiVolnyExemplar(string ISBN)
    {
        return colExemplars.Find(e => e.ISBN == ISBN && !e.isLoaned).FirstOrDefault();
    }

    public int PocetVolnychExemplaru(string ISBN)
    {
        return (int)colExemplars.CountDocuments(e => e.ISBN == ISBN && !e.isLoaned);
    }

    public Rezervace VytvorRezervaci(User ctenar, string ISBN)
    {
        int poradi = (int)colRezervace.CountDocuments(r => r.ISBN == ISBN && !r.splnena) + 1;
        Rezervace rez = new Rezervace(ctenar, ISBN, poradi);
        colRezervace.InsertOne(rez);
        return rez;
    }

    public void AktualizujRezervacePoDostupcnosti(string ISBN, IMongoCollection<Rezervace> colRezervace)
    {
        var prvni = colRezervace.Find(r => r.ISBN == ISBN && !r.splnena).SortBy(r => r.poradí).FirstOrDefault();
    ...
    {
        decimal celkem = 0;
        var vypujcky = colVypujcky.Find(v => v.ctenar.Id == ctenar.Id).ToList();

        foreach (var v in vypujcky)
        {
